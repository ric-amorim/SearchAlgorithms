...
public class AMisplaced {
    ...
    public static int custo(Board game) {
        int res = 0;
        String count = game.gps;
    ...
    public static void GM(Board game) {
        PriorityQueue<Board> wcf = new PriorityQueue<>(
                (Board c1, Board c2) -> Integer.compare(foradositio(c1) + custo(c1), foradositio(c2) + custo(c2)));
        if (!game.temSolucao()) {
